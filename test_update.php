...
if($_FILES['photo']['name'] != ''){
  $select_photo = "SELECT test_img FROM test WHERE id=$user_id";
  $del_photo_query = mysqli_query($db_connection,$select_photo);
  $after_assoc = mysqli_fetch_assoc($del_photo_query);
  $delete_from_location = "uploads/test/".$after_assoc['test_img'];
  ...
  if(in_array($extention, $allowed_extention)){
    if($uploaded_file['size'] <= 2000000000){
      ...
      $update = "UPDATE test SET test_img='$file_name' WHERE id=$user_id";
      $photo_uploaded = mysqli_query($db_connection,$update);

      $update_query = "UPDATE test SET test_title='$name', test_des='$email' WHERE id=$user_id";
